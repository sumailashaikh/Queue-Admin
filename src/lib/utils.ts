...
export function formatCurrency(
  amount: number,
  currencyCodeParam: string = "USD",
  language: string = "en",
) {
  const baseLang = (language || "en").split("-")[0].toLowerCase();
  const param =
    currencyCodeParam != null ? String(currencyCodeParam).trim() : "";
  let currencyCode =
    param !== "" && param.toLowerCase() !== "null" && /^[A-Z]{3}$/i.test(param)
      ? param.toUpperCase()
      : "USD";
...
export function formatDuration(
  ...
  if (remainingMinutes === 0) {
    return t(hours === 1 ? "common.duration_h" : "common.duration_hs", {
      h: hours,
    });
  }

  return t(hours === 1 ? "common.duration_hm" : "common.duration_hsm", {
    h: hours,
    m: remainingMinutes,
  });
}
